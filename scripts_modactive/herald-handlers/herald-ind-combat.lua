...
-- Returns true if the pinned HF has combat announcements enabled.
local function combat_enabled(hf_id)
...
local function handle_simple_battle(ev, dprint)
    ...
    for i = 0, #group1 - 1 do
        ...
        if pinned[hf_id] and combat_enabled(hf_id) then
...
local function handle_wounded(ev, dprint)
    ...
    if pinned[woundee] and combat_enabled(woundee) then
        fire(ev.id, ('%s was wounded.'):format(hf_name(woundee)), dprint)
    elseif pinned[wounder] and combat_enabled(wounder) then
...
local function handle_attacked_site(ev, dprint)
    ...
    if pinned[attacker] and combat_enabled(attacker) then
...
local function handle_war_field_battle(ev, dprint)
    ...
    if pinned[att_gen] and combat_enabled(att_gen) then
        fire(ev.id, ('%s led forces in battle.'):format(hf_name(att_gen)), dprint)
    elseif pinned[def_gen] and combat_enabled(def_gen) then
...
local function handle_war_attacked_site(ev, dprint)
    ...
    elseif pinned[att_hf] and combat_enabled(att_hf) then
...
local function handle_body_abused(ev, dprint)
    ...
    if pinned[abuser] and combat_enabled(abuser) then
...
local function handle_hf_died_as_kill(ev, dprint)
    ...
    if not pinned[slayer] or not combat_enabled(slayer) then return end
...
local function handle_entity_overthrown(ev, dprint)
    ...
    if pinned[overthrown] and combat_enabled(overthrown) then
        fire(ev.id, ('%s was overthrown.'):format(hf_name(overthrown)), dprint)
    elseif pinned[taker] and combat_enabled(taker) then
        fire(ev.id, ('%s seized power.'):format(hf_name(taker)), dprint)
    elseif pinned[instigator] and combat_enabled(instigator) then
...
local function handle_poll(dprint)
    ...
    for i = 0, #active - 1 do
        ...
        if not combat_enabled(hf_id) then
        ...
        local baseline = combat_baselines[unit.id]
        ...
        if count > baseline then
            -- New combat reports since last poll
            local name = hf_name(hf_id)
            local new_count = count - baseline
            combat_baselines[unit.id] = count

            -- Try to identify the opponent from the unit's name in the report
            local opponent = nil
            ...
            if rep_id then
                ...
                if ok_rep and rep then
                    ...
                    if ok_txt and txt and txt ~= '' then
                        -- DF reports: "The X verbs the Y ..." - extract Y as opponent
                        local unit_name_lower = name:lower()
                        -- If the pinned HF is the subject, opponent is after the verb
                        -- If the pinned HF is the object, opponent is the subject
                        local subj, obj = txt:match('^The (.+) %a+ the (.+)[,!.]')
                        if subj and obj then
                            if subj:lower():find(unit_name_lower, 1, true) then
                                opponent = obj
                            elseif obj:lower():find(unit_name_lower, 1, true) then
                                opponent = subj
                            end
                        end
                    end
